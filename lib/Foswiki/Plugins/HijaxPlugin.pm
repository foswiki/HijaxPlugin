...
use vars
  qw( $VERSION $RELEASE $SHORTDESCRIPTION $debug $pluginName $NO_PREFS_IN_TOPIC $base $defaultsAdded );
...
sub addDefaultsToPage {
    ...
    $defaultsAdded = 1;
    Foswiki::Func::addToZone( 'head', 'HIJAXPLUGIN_CSS',
...
HERE
    ...
    my $configurableInit =
      Foswiki::Func::expandTemplate('hp_configurable_init');
    Foswiki::Func::addToZone( 'script', 'HIJAXPLUGIN_JS',
        <<"HERE", 'JQUERYPLUGIN::FOSWIKI,JQUERYPLUGIN::BGIFRAME,JQUERYPLUGIN::UI,JQUERYPLUGIN::HOVERINTENT' );
$configurableInit
...
sub _noTemplateMessage {
    my $tmpl = shift;
    return
"<div class='foswikiAlert'>Warning: a template named $tmpl has not been defined for this interface.</div>";
}
...
sub completePageHandler {
    ...
    return
      unless $query->param('cover')
          && $query->param('cover') =~ /ajax/
          && $_[0] =~ /%TEMPLATE:/;
    ...
    if ( Foswiki::Func::getContext()->{'login'} ) {
        ...
        $tmplp[0] = 'content'
          unless ( $hijax[0] || $tmplp[0] )
          ;    # if nothing else, we return the 'content' tmpl
        unless ( grep ( /content/, @tmplp ) ) {
            push( @tmplp, 'content' )
              if ( Foswiki::Func::expandTemplate('context') =~ /oops/ );
        }
    }
...
# need to get the addToZone content added before to_json is called otherwise it won't be json
        Foswiki::Func::writeDebug(
            "HijaxPlugin specifically calling ZonePlugin::completePageHandler")
          if $debug;
    ...
    if ($templatetext) {
        $templatetext =
          Foswiki::Func::renderText(
            Foswiki::Func::expandCommonVariables($templatetext) );
    }
    else {
...
sub redirectCgiQueryHandler {
    ...
    if ($debug) {
        ...
        Foswiki::Func::writeDebug(
            "- ${pluginName}::redirectCgiQueryHandler( $query_string, $_[1] )");
    }
...
# the result of a successful login so leave the core to unpack all the params from the cache
    return 0
      if $_[1] =~ /foswiki_(redirect_cache|origin)/ && $_[1] !~ /\/login\//;

    my $append = ( $_[1] =~ m/(\?.*)$/ ) ? ';' : '?';
    $_[1] .= $append . 'cover=' . $query->param('cover');
    $_[1] .= ';hijax=' . join( ';hijax=', $query->param('hijax') )
      if $query->param('hijax');
    $_[1] .= ';tmplp=' . join( ';tmplp=', $query->param('tmplp') )
      if $query->param('tmplp');
    if (   $query->param('returntemplate')
        && $_[1] !~ /oops(accessdenied|alerts)/ )
    {
